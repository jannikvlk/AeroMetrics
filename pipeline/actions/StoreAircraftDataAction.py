...
def extract(message: str) -> str | None:
        ...
        data["Corrections"].append(
            {
                ...
                "Pos": (
                    match.group(4).strip() if match.group(4).strip() != "NULL" else None
                ),
    ...
    weight_table_pattern = re.compile(
        r"(Pax|Special Pax|Bag)\s*\|\s*([A-Z]*)\s*\|\s*(.*?)(?:\s*KG|NULL)"
    )
    ...
    # Extract max weights
    max_weights_pattern = re.compile(
        r"(\w+ Adj\.)\s*:\s*(NULL|\d+\.\d+\s*KG)?\s*\|\s*(NULL|\d+\.\d+\s*KG)?"
    )
    ...
    data["Status"] = {
        ...
        "ALLOWANCE_CHECK_PERFORMED": None,
        "AUTOMATION_STARTED": None,
        "AUTO_MODE_ACTIVE": None,
        "BAG_LOAD_ITEMS_GEN": None,
        "BAG_ULD_ORD": None,
        "CABIN_CONFIG": None,
        "CALC_HIST_DATA": None,
        "CARGO_FINAL": None,
